...
suspend fun <S, E> ContainerHandler<S, E>.collectState(block: (S) -> Unit): Unit = container.collectState(block)

suspend fun <S, E> ContainerHandler<S, E>.collectEffect(block: (E) -> Unit): Unit = container.collectEffect(block)
